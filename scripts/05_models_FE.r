# ==============================================================================
# # 05_models.R — Zero-Inflated NB GLMM + Marginal Effects (MIPEX FE Version)
...
# ==============================================================================
# 2. NUOVA PULIZIA STRUTTURALE (Manteniamo i paesi con CMP/Discrimination mancanti!) ####
# ==============================================================================
message("   Pulizia preventiva dei paesi strutturalmente privi di dati CMP (necessario per l'interazione)...")

# Controlliamo SOLO il CMP, perché ci serve tassativamente per l'interazione nel Modello 3
paesi_da_escludere <- df_merged_3 %>%
  ...
  filter(pct_na_cmp == 1) %>% # Escludiamo solo se manca il CMP al 100%
  pull(country)

message(paste("🚫 Paesi esclusi strutturalmente (Mancanza CMP):", paste(paesi_da_escludere, collapse = ", ")))

# Creazione del dataset finale per i modelli
df_model_data <- df_merged_3 %>% 
  filter(legislative_term %in% c("8th", "9th")) %>% 
  filter(!country %in% paesi_da_escludere) %>%
  drop_na(
    pqs_discrimination, total_questions, gender, mep_id, country,
    party_family, libe, minority, misery_index, cmp_center
    # Rimosse mipex_center e discrimination_center dalla drop_na()!
  )

message(paste("✅ N finale del dataset RIPOPOLATO per i modelli:", nrow(df_model_data)))


# ==============================================================================
# 3. NUOVA STIMA CON EFFETTI FISSI DI PAESE (Risoluzione Definitiva NaN) ####
# ==============================================================================
...
# Modello 1: Baseline con FE di Paese
message("   Fitting zinb_model1 (Country FE)...")
zinb_model1 <- glmmTMB(
   pqs_discrimination ~ 
       gender + misery_index + party_family + libe + 
       offset(log(total_questions + 1)) + 
       (1 | mep_id) + factor(country),   # Country RE rimosso, inserito factor()
   family = nbinom2,
   ziformula = ~ 1,            
   data = df_model_data,
...
# Modello 2: Aggiunta Minority
message("   Fitting zinb_model2 (Country FE)...")
zinb_model2 <- glmmTMB(
   pqs_discrimination ~ 
       gender + misery_index + party_family + libe + 
       minority +
       offset(log(total_questions + 1)) + 
       (1 | mep_id) + factor(country),
   family = nbinom2,
   ziformula = ~ 1,            
   data = df_model_data,
   control = controllo_ottimizzatore
...
# Modello 3: Interazione Specifica (Minority × MIPEX)
message("   Stima del Modello 3 (Interazione con Country FE)...")
zinb_model3 <- glmmTMB(
  pqs_discrimination ~ 
    gender + misery_index + party_family + libe + 
    minority + minority:cmp_center + # Solo interazione, l'effetto principale è nel FE
    offset(log(total_questions + 1)) + 
    (1 | mep_id) + factor(country),
  family = nbinom2,
  ziformula = ~ 1,           
...
model_comparison <- performance::compare_performance(
  zinb_model1, zinb_model2, zinb_model3,
...
essential_comparison <- model_comparison %>%
  ...
  mutate(
    Model_Label = case_when(
      ...
      Name == "zinb_model3" ~ "M3: Fully Controlled + Minority*CMP Interaction",
      TRUE ~ Name
    ),
    RMSE = round(RMSE, 3),
    AIC = round(AIC, 1),
...
anova_results <- anova(zinb_model1, zinb_model2, zinb_model3, test = "Chisq")
...
# ==============================================================================
message("   Estrazione e formattazione dei coefficienti per i 3 modelli...")
...
write_csv(tabella_coefficienti, here("output", "model_coefficients_comparisonFE.csv"))
message("🎉 Tabella dei coefficienti salvata in: output/model_coefficients_comparisonFE .csv")
...
tabella_compatta <- tabella_coefficienti %>%
  mutate(
    ...
    `M3 (+ MIPEX interaction)`    = if_else(!is.na(estimate_M3), paste0(estimate_M3, " (IRR: ", exp_estimate_M3, ") ", pval_str_M3), "-"),
  ) %>%
  dplyr::select(Predictor, `M1 (Baseline)`, `M2 (+ Minority)`, `M3 (+ MIPEX interaction)`)
