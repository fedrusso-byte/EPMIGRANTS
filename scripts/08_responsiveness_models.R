...
# Author: Marcello Carammia (Modified for Misery Index)
...
model_data <- resp_panel %>%
  ...
  mutate(
    ...
    mip_eu_mean_z = as.numeric(scale(mip_eu_mean)),
    mip_deviation_z = as.numeric(scale(mip_deviation))
  )

# AGGIORNAMENTO: Check per misery_index invece di unemployment
has_problems <- "misery_index" %in% names(model_data) &&
  !all(is.na(model_data$misery_index))

if (has_problems) {
  model_data <- model_data %>%
    mutate(
      misery_eu_mean_z = as.numeric(scale(misery_eu_mean)),
      misery_deviation_z = as.numeric(scale(misery_deviation)),
      asylum_eu_mean_z = as.numeric(scale(asylum_eu_mean)),
      asylum_deviation_z = as.numeric(scale(asylum_deviation))
    )
}
...
if (has_problems) {
  model_data_problems <- model_data %>%
    dplyr::filter(!is.na(misery_eu_mean_z), !is.na(asylum_eu_mean_z))
}
...
if (has_problems) {
  ...
  summaries$m1 <- fit_and_cleanup(
    n_questions ~ mip_share_z + misery_eu_mean_z + misery_deviation_z +
      asylum_eu_mean_z + asylum_deviation_z +
      issue_name + semester_id +
      committee_match + gender + geographic_region + euro_member + party_family +
      (1 | mep_id) + (1 | country) + (1 | group_abbr) +
      offset(log_offset),
    data = model_data_problems,
    model_name = "m1",
...
m2_formula <- if (has_problems) {
  n_questions ~ mip_eu_mean_z + mip_deviation_z +
    misery_eu_mean_z + misery_deviation_z +
    asylum_eu_mean_z + asylum_deviation_z +
    issue_name + semester_id +
    committee_match + gender + geographic_region + euro_member + party_family +
    (1 | mep_id) + (1 | country) + (1 | group_abbr) +
...
summaries$m2 <- fit_and_cleanup(
  m2_formula,
  data = if (has_problems) model_data_problems else model_data,
  model_name = "m2",
...
m3_formula <- if (has_problems) {
  n_questions ~ mip_eu_mean_z * domain + mip_deviation_z * domain +
    misery_eu_mean_z + misery_deviation_z +
    asylum_eu_mean_z + asylum_deviation_z +
    issue_name + semester_id +
    committee_match + gender + geographic_region + euro_member + party_family +
...
# Domain-specific slopes (emtrends) — need m3 in memory
message("\n--- Domain-specific effects (emtrends) ---")
tryCatch({
  trends_eu <- emtrends(m3, ~ domain, var = "mip_eu_mean_z",
                        nuisance = c("semester_id", "issue_name"))
  trends_dev <- emtrends(m3, ~ domain, var = "mip_deviation_z",
                         nuisance = c("semester_id", "issue_name"))
  domain_trends <- list(
    eu_mean = trends_eu, deviation = trends_dev,
    eu_mean_pairs = pairs(trends_eu), deviation_pairs = pairs(trends_dev)
  )
  saveRDS(domain_trends, here("output", "resp_domain_trends.rds"))
  message("  Domain trends saved")
}, error = function(e) {
...
if (any(!is.na(model_data$system_type))) {
  ...
  summaries$m4 <- fit_and_cleanup(
    n_questions ~ mip_eu_mean_z * system_type + mip_deviation_z * system_type +
      issue_name + semester_id +
      committee_match + gender + geographic_region + euro_member + party_family +
      (1 | mep_id) + (1 | country) + (1 | group_abbr) +
      offset(log_offset),
...
key_terms <- c("mip_eu_mean_z", "mip_deviation_z")
if (has_problems) {
  key_terms <- c(key_terms, "misery_eu_mean_z", "misery_deviation_z",
                 "asylum_eu_mean_z", "asylum_deviation_z")
}
...
forest_data <- irr_m2 %>%
  ...
  mutate(
    term_label = case_when(
      term == "mip_eu_mean_z"      ~ "MIP: EU-wide",
      term == "mip_deviation_z"    ~ "MIP: Domestic deviation",
      term == "misery_eu_mean_z"   ~ "Misery Index: EU-wide",
      term == "misery_deviation_z" ~ "Misery Index: Domestic deviation",
      term == "asylum_eu_mean_z"   ~ "Asylum: EU-wide",
      term == "asylum_deviation_z" ~ "Asylum: Domestic deviation",
      TRUE ~ term
    ),
